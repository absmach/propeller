...
5. **[Restart Services After Provisioning](#step-5-restart-services-after-provisioning)** - Restart services to pick up new credentials
...
docker compose -f docker/compose.yaml -f examples/fl-demo/compose.yaml --env-file docker/.env restart manager coordinator-http proplet proplet-2 proplet-3 proxy
...
> ```bash
> docker compose -f docker/compose.yaml -f examples/fl-demo/compose.yaml --env-file docker/.env up -d manager coordinator-http proplet proplet-2 proplet-3 proxy
> ```
>
...
- **Step 5**: Restart Services After Provisioning - Only after provisioning or credential changes
...
1. **Add proxy configuration to `docker/.env`**:
   ...
   PROXY_REGISTRY_PASSWORD=ghp_xxxxx  # Your GitHub PAT token
