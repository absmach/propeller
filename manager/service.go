...
func (svc *service) ListProplets(ctx context.Context, offset, limit uint64, status string) (proplet.PropletPage, error) {
	...
	case "", "active", "inactive":
		// valid
	default:
		return proplet.PropletPage{}, fmt.Errorf("invalid proplet status filter: %q", status)
	}

	if status != "" {
		// When filtering by status, fetch all records so we can filter
		// accurately and then apply offset/limit to the filtered set.
		allProplets, _, err := svc.propletRepo.List(ctx, 0, math.MaxInt64)
		if err != nil {
			return proplet.PropletPage{}, err
		}
		for i := range allProplets {
			allProplets[i].SetAlive()
		}

		filtered := make([]proplet.Proplet, 0, len(allProplets))
		//nolint:gocritic
		for _, p := range allProplets {
			if (status == "active" && p.Alive) || (status == "inactive" && !p.Alive) {
				filtered = append(filtered, p)
			}
		}

		filteredTotal := uint64(len(filtered))
		start := min(offset, filteredTotal)
		end := min(start+limit, filteredTotal)

		return proplet.PropletPage{
			Offset:   offset,
			Limit:    limit,
			Total:    filteredTotal,
			Proplets: filtered[start:end],
		}, nil
	}
