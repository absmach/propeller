...
func NewService(
	...
	svc.coordinator = NewWorkflowCoordinator(tasksDB, svc, logger)
	...
	for i := range tasks {
		...
		if err := svc.tasksDB.Create(ctx, t.ID, t); err != nil {
			for j := range createdTasks {
				...
				_ = svc.tasksDB.Delete(ctx, created.ID)
			}
...
func (svc *service) ListJobs(ctx context.Context, offset, limit uint64) (JobPage, error) {
	data, _, err := svc.tasksDB.List(ctx, 0, 10000)
	if err != nil {
		return JobPage{}, err
	}
	...
	for i := range data {
		...
		if t.JobID != "" {
			jobMap[t.JobID] = append(jobMap[t.JobID], t)
		}
	}
...
func (svc *service) StartJob(ctx context.Context, jobID string) error {
	...
	executionMode := "configurable" // default
