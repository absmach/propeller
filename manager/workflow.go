...
func NewWorkflowCoordinator(tasksDB storage.Storage, service Service, logger *slog.Logger) *WorkflowCoordinator {
...
func (wc *WorkflowCoordinator) CheckAndStartReadyTasks(ctx context.Context, workflowID string) error {
	...
	for i := range readyTasks {
		...
		if !shouldRun {
			...
			if err := wc.tasksDB.Update(ctx, t.ID, t); err != nil {
...
func (wc *WorkflowCoordinator) getWorkflowTasks(ctx context.Context, workflowID string) ([]task.Task, error) {
	data, _, err := wc.tasksDB.List(ctx, 0, 10000)
	if err != nil {
		return nil, err
	}
