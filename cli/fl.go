...
var flCmd = []cobra.Command{
	{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if roundID == "" {
				logErrorCmd(*cmd, errors.New("round-id is required"))

				return
			}
			if modelURI == "" {
				logErrorCmd(*cmd, errors.New("model-uri is required"))

				return
			}
			if taskWasmImage == "" {
				logErrorCmd(*cmd, errors.New("task-wasm-image is required"))

				return
			}
			if len(participants) == 0 {
				logErrorCmd(*cmd, errors.New("at least one participant is required"))

				return
			}

			roundStart := map[string]any{
				"round_id":        roundID,
				"model_uri":       modelURI,
			...
			_, _ = os.Stdout.WriteString("Round start message (publish to fl/rounds/start):\n")
			_, _ = os.Stdout.WriteString(string(roundStartJSON))
